...
- **FR-041** — Changing the active connection MUST stop current audio, cancel/invalidate queued or in-flight narration, clear stale read-along state, then route subsequent synthesis exclusively to the selected connected service. Native playback-start/finish signals MUST carry a generation so a delayed old finish cannot stop or advance the new route. It MUST NOT require a restart or credential re-entry.
