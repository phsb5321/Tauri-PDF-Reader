...
- [x] T008 — Branch from merged Slice A and add `.github/workflows/macos-flake.yml`: protected-main-only read-only build under the repo-scoped M5's isolated runner account. (FR-006, FR-007)
- [x] T009 — Add malformed-bundle, no-PR-trigger, read-only-token, and locked-input controls without exposing signing/application credentials. (FR-005–FR-007)
