...
- [ ] **T001** — Create `.github/workflows/release.yml`: `on: push: tags: ['v*']`,
...
- [ ] **T002** — Add the setup steps mirroring `ci.yml`: apt system deps (+
  `fakeroot dpkg file`), node 20, pnpm 10, Rust stable, cargo cache, and
  `pnpm install` before the build. (FR-008)
- [ ] **T003** — Add the `tauri-apps/tauri-action` publish step, SHA-pinned to
...
- [ ] **T004** — `actionlint .github/workflows/release.yml` → no errors. (SC-004)
- [ ] **T005** — `git diff --stat` confirms the diff touches only
...
- [ ] **T006** — Push branch, then push a throwaway `v0.1.0-rc-test` tag from the
  branch; poll `gh run list --workflow=Release` until the run reaches
  conclusion `success`. (SC-001)
- [ ] **T007** — `gh release view v0.1.0-rc-test` confirms exactly two assets
  ending in `.AppImage` and `.deb`. (SC-002)
- [ ] **T008** — Confirm no release run fired for the branch's ordinary commits
  (only the tag triggered it). (SC-005)
- [ ] **T009** — Clean up: `gh release delete v0.1.0-rc-test` + delete the remote
  and local `v0.1.0-rc-test` tag. Leave no throwaway artifacts.

## Phase 4 — Review, docs, ship

- [ ] **T010** — Adversarial cross-family gate on the diff (codex first;
  usage-limited until 25/07 → `glm-review`); address BLOCKER/MAJOR findings.
- [ ] **T011** — Update `docs/agent-backlog-state.md` with a new Iteration
  closing the release-pipeline Tier-3 item.
- [ ] **T012** — Open PR (green CI + review-clean). Because this touches
