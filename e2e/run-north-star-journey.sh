...
# /var/tmp/tmp.* profile as outside that process temp root.
...
toolchain_exec '
  ...
  EXPECTED_NEGATIVE_CONTROLS="[\"silent_no_key\",\"play_not_crossing_boundary\",\"lingering_original_process\",\"wrong_resumed_document_page\",\"missing_highlight\"]"
  for phase in no-key-open configured-close resume-verify; do
    jq -e --arg phase "$phase" --arg sha "$NORTH_STAR_SOURCE_SHA" --argjson controls "$EXPECTED_NEGATIVE_CONTROLS" \
      ".phase == \$phase and .source_sha == \$sha and .negative_controls == \$controls and all(.steps[]; .result == \"pass\" and .failure_reason == null)" \
