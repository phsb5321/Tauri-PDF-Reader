...
| reader        | `e2e/run-reader-journey.sh`          | Reading position lost across restart; navigation, page input, or zoom commit broken                   | Position survives a genuine restart; nav moves the rendered page; page input works; the single exact zoom label advances only with an exact ready render ticket                                                               | Fixture only; pointer-anchor/high-zoom geometry live in the feature-scoped library lane                          |
...
## Feature-scoped completion lanes (not standing matrix authority)

| Lane                 | Runner                                                                                                                                          | User risk it gates                                                                                                                              | Key assertions                                                                                                                                                                                                        |
| -------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| narration cockpit    | `scripts/e2e-narration-cockpit.sh` → reused `scripts/e2e-local-tts.sh`                                                                          | Cockpit tabs/settings are decorative; read-from-here resumes stale audio; paragraph action starts the wrong source; page turn blocks fresh Play | Four public tabs; Continuous/normalization/language; exact word; Pause → physical excerpt selection → replacement; Stop; professional paragraph action → exact chosen paragraph; manual next page → immediate Play    |
| paragraph overlay    | `E2E_SPEC=./e2e/paragraph-overlay.e2e.mjs VITE_E2E_NATIVE_PDF_URL=/e2e-prosody-fixture.pdf VITE_E2E_NATIVE_TTS=none bash scripts/e2e-native.sh` | Margin markers overlap text/each other, lose keyboard focus, or drift at zoom                                                                   | Initial fit-page targets never overlap; 100%/300% painted tick and icon stay 7–17px before and ≤1px vertically from the first line; ≥44px target; paper marker; visible focus; backing side ≤8192                     |
| library completeness | `scripts/e2e-library-completeness.sh`                                                                                                           | Real PDF drag/session and continuous high zoom fail despite state strings changing                                                              | Physical Ctrl+wheel; exact selected/committed percentage; canvas/text/page ratio and backing geometry; normalized pointer anchor; no residual preview; public 300% commit ≤8192; narrow toolbar; real OS drag/session |

These lanes are completion evidence for their feature branches. They reuse the
base-owned toolchain/profile/driver but are not silently added to the protected
eight-lane workflow; promoting one requires the same trust-anchor workflow
change and therefore remains Pedro-gated.
