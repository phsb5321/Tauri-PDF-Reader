...
- `src-tauri/tests/frontend_schema_contract.rs` reads the production DDL out of
  ...
  ships.
- The view is dropped before each launch's migrations and recreated after them,
  so a database that has been opened once by a given build has that build's
  definition. It is not a versioned migration: there is no profile out there
  running an older view.
