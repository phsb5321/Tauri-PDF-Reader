...
export function useTtsPrebuffer(
  ...
  const prebufferPage = useCallback(
    async (pageNum: number): Promise<boolean> => {
      ...
      try {
        ...
        console.debug(
          "[TtsPrebuffer] Pre-buffering page",
          pageNum,
          `(${text.length} chars)`,
        );
    ...
    [getPageText, provider, selectedVoiceId],
  );

  /**
   * Pre-buffer current page and lookahead pages
   */
  const prebufferPages = useCallback(
    async (startPage: number, totalPages: number) => {
      if (!initialized || provider !== "elevenlabs" || !pdfDocument) return;
      ...
      console.debug(
        "[TtsPrebuffer] Queuing pages for pre-buffer:",
        pagesToBuffer,
      );
    ...
    [initialized, provider, pdfDocument, lookahead, prebufferPage],
  );
   ...
   */
  useEffect(() => {
    if (!enabled || !initialized || provider !== "elevenlabs" || !pdfDocument)
      return;
