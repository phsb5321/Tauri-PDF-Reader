...
vi.mock("../../services/pdf-service", () => ({
  pdfService: {
    loadDocumentForCover: (...args: unknown[]) => loadDocumentForCover(...args),
    getPage: vi.fn(),
    renderPage: vi.fn(),
  },
}));
...
const DOC = makeDoc(
  "a1b2c3d4e5f60718293a4b5c6d7e8f9012345678abcd1234ef567890abcde",
  "Paper One",
);
...
function coverWrapper(accessibleImage: HTMLElement): HTMLElement {
  const wrapper = accessibleImage.closest<HTMLElement>(".document-cover");
  if (!wrapper) throw new Error("accessible cover image has no wrapper");
  return wrapper;
}
...
describe("cover pipeline contract (121)", () => {
  it("renders the deterministic fallback with the accessible name before any generation", () => {
    const doc = makeDoc(
      "11a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1",
    );
    ...
    // A native image/SVG is named by the title in every state.
    const cover = coverWrapper(getByRole("img", { name: "Paper One" }));
  ...
  it("is deterministic: the same id renders the same fallback class on every mount", () => {
    ...
    const seedC = c.container
      .querySelector(".document-cover")
      ?.getAttribute("data-seed");
    const seedA = a.container
      .querySelector(".document-cover")
      ?.getAttribute("data-seed");
  ...
  it("warm cache: a second mount of the same id runs extraction exactly once", async () => {
    const doc = makeDoc(
      "22b2b2b2b2b2b2b2b2b2b2b2b2b2b2b2b2b2b2b2b2b2b2b2b2b2b2b2b2b2b2b2b2",
    );
    stubCanvasToBlob();
    ...
    await waitFor(() =>
      expect(
        coverWrapper(b.getByRole("img", { name: "Paper One" })).dataset.state,
      ).toBe("ready"),
    );
  ...
  it("no layout shift: the placeholder and the raster share one wrapper element", async () => {
    const doc = makeDoc(
      "33c3c3c3c3c3c3c3c3c3c3c3c3c3c3c3c3c3c3c3c3c3c3c3c3c3c3c3c3c3c3c3c3",
    );
    stubCanvasToBlob();
    ...
    act(() => intersect());
    await waitFor(() =>
      expect(wrapperBefore?.getAttribute("data-state")).toBe("ready"),
    );
    ...
    expect(
      wrapperAfter?.querySelector("img.document-cover-img"),
    ).not.toBeNull();
  });

  it("lazy: zero repository calls while off-screen; generation starts on intersection", async () => {
    const doc = makeDoc(
      "44d4d4d4d4d4d4d4d4d4d4d4d4d4d4d4d4d4d4d4d4d4d4d4d4d4d4d4d4d4d4d4d4",
    );
    stubCanvasToBlob();
    mockRepoGet.mockResolvedValue(null);
    mockRepoStore.mockResolvedValue(undefined);
  ...
  it("failed generation writes nothing: corrupt source keeps the fallback and never stores", async () => {
    const doc = makeDoc(
      "55e5e5e5e5e5e5e5e5e5e5e5e5e5e5e5e5e5e5e5e5e5e5e5e5e5e5e5e5e5e5e5e5",
    );
    mockRepoGet.mockResolvedValue(null);
    ...
    await waitFor(() =>
      expect(
        coverWrapper(getByRole("img", { name: "Paper One" })).dataset.state,
      ).toBe("fallback"),
    );
    expect(mockRepoStore).not.toHaveBeenCalled();
    // The deterministic fallback is still there (never a blank box).
    expect(
      coverWrapper(getByRole("img", { name: "Paper One" })).querySelector(
        ".cover-fallback",
      ),
    ).not.toBeNull();
  });
  ...
  it("size bound: an oversized source is rejected BEFORE any file read (NC)", async () => {
    const doc = makeDoc(
      "66f6f6f6f6f6f6f6f6f6f6f6f6f6f6f6f6f6f6f6f6f6f6f6f6f6f6f6f6f6f6f6f6",
    );
    mockRepoGet.mockResolvedValue(null);
  ...
  it("relocation resets the pipeline: a healed path reaches ready on the same id", async () => {
    ...
    const doc = makeDoc(
      "88b8b8b8b8b8b8b8b8b8b8b8b8b8b8b8b8b8b8b8b8b8b8b8b8b8b8b8b8b8b8b8b8",
    );
