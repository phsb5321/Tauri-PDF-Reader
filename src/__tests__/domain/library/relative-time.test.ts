...
describe("formatRelativeReadTime", () => {
  ...
  it("says today for the same calendar day", () => {
    expect(formatRelativeReadTime("2026-08-04T09:00:00Z", NOW)).toBe("today");
  });

  it("says today for a timestamp that is technically in the future (clock skew)", () => {
    expect(formatRelativeReadTime("2026-08-05T12:00:00Z", NOW)).toBe("today");
  });

  it("says yesterday for the previous calendar day", () => {
    expect(formatRelativeReadTime("2026-08-03T09:00:00Z", NOW)).toBe(
      "yesterday",
    );
  });

  it("counts days for the rest of the week", () => {
    expect(formatRelativeReadTime("2026-08-01T12:00:00Z", NOW)).toBe(
      "3 days ago",
    );
  });

  it("keeps counting days at a week boundary rather than switching to weeks", () => {
    expect(formatRelativeReadTime("2026-07-28T12:00:00Z", NOW)).toBe(
      "7 days ago",
    );
  });

  it("keeps counting days at a month boundary rather than switching to months", () => {
    expect(formatRelativeReadTime("2026-07-05T12:00:00Z", NOW)).toBe(
      "30 days ago",
    );
  });

  it("keeps counting days for a very stale book", () => {
    expect(formatRelativeReadTime("2026-06-05T12:00:00Z", NOW)).toBe(
      "60 days ago",
    );
  });
});
