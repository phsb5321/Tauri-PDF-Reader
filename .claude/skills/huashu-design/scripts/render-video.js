...
if (!HTML_FILE || HTML_FILE.startsWith("--")) {
  ...
  console.error(
    "Example: NODE_PATH=$(npm root -g) node render-video.js my-animation.html",
  );
  process.exit(1);
}
...
console.log(
  `  size: ${WIDTH}x${HEIGHT} · duration: ${DURATION}s · hide-chrome: ${!KEEP_CHROME}`,
);
...
(async () => {
  ...
  if (!KEEP_CHROME) {
    await recordCtx.addInitScript((css) => {
      ...
      function hideChromeBars() {
        ...
        document
          .querySelectorAll("div, nav, header, footer, section, aside")
          .forEach((el) => {
  ...
  if (hasReady) {
    ...
    console.log("");
    console.log(
      `  ⚠️  WARNING: window.__ready signal not detected within ${READY_TIMEOUT}s`,
    );
    console.log(
  ...
  console.log(
    `▸ WebM: ${(fs.statSync(webmPath).size / 1024 / 1024).toFixed(1)} MB`,
  );
