...
| --------------------------------------------------- | ---------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------- |
| PDF-derived page text (the page you ask to be read) | ElevenLabs (`https://api.elevenlabs.io/v1`)                      | Play, resume-and-play, auto-page continuation, or marked-provider prebuffer while the connected ElevenLabs route is active                 | `src-tauri/src/ai_tts/elevenlabs.rs` (reqwest client; optional `elevenlabs-tts` feature)                             |
| PDF-derived page text (the page you ask to be read) | Groq (`https://api.groq.com/openai/v1/audio/speech`)             | Play, Read from here, or auto-page continuation while the connected Groq route is active; connecting/switching alone sends no PDF text     | `src-tauri/src/adapters/groq_tts.rs` (pinned origin/model, bounded bearer-authenticated WAV client)                  |
| PDF-derived page text (the page you ask to be read) | The exact user-operated loopback service `http://127.0.0.1:5301` | Play, Read from here, resume-and-play, or auto-page continuation while Local TTS is active; local mode never prebuffers before public Play | `src-tauri/src/adapters/local_tts.rs` (bounded reqwest client, exact destination whitelist, idempotent WAV contract) |
...
The in-app disclosure names every connection destination. ElevenLabs and Groq
forms state that requested PDF-derived text leaves the device for that named
service. Local mode prints the exact loopback URL and states that text goes
there and not to ElevenLabs (`src/components/playback-bar/AiTtsSettings.tsx`).

**Diagnostics:** the debug-logs surface (`DebugLogs.tsx`, fed by
`get_debug_logs`/`export_debug_logs`) redacts at the export boundary —
user paths (`/home/<redacted-user>/…`) and cloud secret-shaped tokens
(`sk_<redacted>` / `gsk_<redacted>`) cannot survive into display or clipboard
...
ElevenLabs and Groq API keys are **session-only by design** (#73):

- They are excluded from persisted state — `partialize` stores only non-secret
  per-provider voice preferences, speed, and auto-page state; legacy persisted
  keys are stripped
  by `migrate`/`merge` before application effects can see them.
- A submitted key is cleared from the WebView form after connection and remains
  only inside the native provider client for the current process;
...
Each key travels only to its named cloud service over TLS as the credential of
the requests in the dataflow table above. Neither is written to SQLite, native
config, the TTS cache, Web storage, or the settings store. Local mode requires no API key;
